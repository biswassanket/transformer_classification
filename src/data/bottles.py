...
def Bottle_dataset(args, embedding_size):
    ...
    with open(args.data_path + '/Drink_Bottle/bottles_local_feats.npy', 'rb') as fp:
        local_feats = np.load(fp, encoding='bytes')

    # Create img_name to index of local features
    with open(args.data_path + '/Drink_Bottle/bottles_local_feats_image_ids.txt', 'r') as fp:
    ...
    # BBOXES LOADING FOR TEXT AND LOCAL FEATURES
    ...
    train_loader = Bottle_Train(args, gt_annotations, text_embedding, embedding_size, local_feats, image_name2features_index, text_bboxes, local_bboxes, train_transform)
    ...
    test_loader = Bottle_Test(args, gt_annotations, text_embedding, embedding_size, local_feats, image_name2features_index, text_bboxes, local_bboxes, test_transform)
...
class Bottle_Train(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.args.embedding == 'w2vec' or self.args.embedding == 'fasttext' or self.args.embedding =='glove':
        ...
        text_bboxes = np.asarray(self.text_bboxes[image_name.replace('images/', '')])
        ...
        return img, label, text_features, local_features, text_bboxes_features, local_bboxes_features, image_name.replace('images/', '')
